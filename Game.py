...
class Game:
                ...
                for event in self.display.get_events():
                    ...
                    if event.type == pygame.QUIT:
                        ...
                        for player in self.table.get_players():
                            ...
                            if True not in [p.get_went_alone() for p in self.table.get_players() 
                                            if ((player.get_team() == p.get_team()) and (player.name != p.name))]:
                                ...
                                if player == self.user_player:
                                    ...
                                    self.display.add_card_to_table(played_so_far[-1].get_img_path(),len(played_so_far),False) ## player_pos = 'bottom'
                                    ...
                                    self.display.add_card_to_table(played_so_far[-1].get_img_path(),len(played_so_far),False) ## player_pos = player.get_pos()
                        ...
                        winning_player = [player for player in self.table.get_players() if player.card_played_this_trick == winning_card][0]
                        winning_team = winning_player.get_team()
                        
                        if partner_went_alone_index is None:
                        ...
                        for player in self.table.get_players():
                            player.add_to_player_team_score(score[player.get_team()])
                            if player.name == winning_player.name:
                                player.add_to_player_score()
                            self.display.display_player_trick_score(player)
                            self.display.pump()
    ...
    def play_round(self):
        ...
        self.display.clear_trick_score()
        self.display.remove_player_called_trump(self.table.get_players())
            ...
            while game_on == True:
                ...
                for event in self.display.get_events():
                    if event.type == pygame.QUIT:
                        ...
                        ## TODO: Sort hand according from best to worst based on flipped card being the trump suit
                        ...
                        for player in self.table.get_players():
                            ...
                            ## Determine if the user is dealer, so they can have an option to discard what they want
                            dealer_index = [player.is_dealer() for player in self.table.get_players()].index(True)
                            user_is_dealer = self.table.get_players()[dealer_index].get_is_user()

                            
                            if player == self.user_player:
                                bet, loner_setting = self.display.select_suit(self.user_player, flipped_card.get_suit())
                                if loner_setting == 'on':
                                    self.user_player.went_alone = True
                            else:
                            ...
                            if bet != '':
                                ...
                                if user_is_dealer:
                                    self.display.select_bet_discard(self.user_player, flipped_card)
                                    break
                        ...
                        if picked_up == False:
                            ...
                            for player in self.table.get_players():
                                ...
                                if player == self.user_player:
                                    bet, loner_setting = self.display.select_suit(self.user_player, all_suits)
                                    if loner_setting == 'on':
                                        self.user_player.went_alone = True
                        ...
                        if trump_suit != '':
                            ...
                            self.display.display_player_called_trump(self.table.get_players())
                            
                            for player in self.table.get_players():
                                player.get_hand().set_trump(trump_suit)

                            if ((picked_up_round == 0) and (not user_is_dealer)):
                            ...
                            for i in range(5):
                                if i == 0:
                                    dealer = self.table.get_players()[dealer_index]
                                    left_of_dealer = self.table.get_player_to_left(dealer)
                                    trick_score, next_leader = self.play_trick(left_of_dealer)
                                    team_0_trick_score = trick_score[0]
                                    team_1_trick_score = trick_score[1]
                                else:
                                    trick_score, next_leader = self.play_trick(next_leader)
                                    team_0_trick_score = trick_score[0]
                                    team_1_trick_score = trick_score[1]
                                
                                score[0] = score[0] + team_0_trick_score
                                print(f"Team 0 round score: {score[0]}")
                                score[1] = score[1] + team_1_trick_score
                                print(f"Team 1 round score: {score[1]}")
